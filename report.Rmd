...
```
```{r populationmap, fig.cap="Map: Victoria Population"}
vicpopulation %>% 
...
Age distribution for Victoria is represent by the density plot, Figure \@ref(fig:agedistributiong57). 
...
```{r agedistributiong57, fig.height=4, fig.cap="Age Distribution"}
...
\newpage
\newpage
\section{Victoria Population: Gender}

To study the association between the topics of study, the figure \ref{fig:populationgender} is a bar chart that shows the population in the sub-divisions of each topic arranged in decreasing order representing both male and female population. The following inferences were made by comparing the individual plots that each represent a topic of study.
...
\begin{figure}[H]
\includegraphics[width=3.5in, height = 4in]{Figures/popedusex-1}
\includegraphics[width=3.5in, height = 4in]{Figures/popindsex-1}
\includegraphics[width=3.5in, height = 4in]{Figures/popfieldsex-1}
\includegraphics[width=3.5in, height = 4in]{Figures/popoccsex-1}
\caption{Population: Gender}
\label{fig:populationgender}
\end{figure}

```{r poptopicsex, include=FALSE}
# <!--
# ### Population by Education
# -->
...
g46a %>%
  ggplot(mapping = aes(x = fct_reorder(afq_level,count_edu_lvl), y = count_edu_lvl, fill = sex)) +
  geom_col(mapping = aes(x = reorder(afq_level,count_edu_lvl), y = count_edu_lvl, fill = sex)) +
  labs(title = "Population Share of education level", y = "Number of students") +
  scale_y_continuous(label=label_number()) +
  theme(axis.title.y = element_blank())+
  coord_flip()
...
\newpage
The Figure \ref{fig:populationtopic} shows the total populations for sub-divisions within each topic.

\begin{figure}[H]
\includegraphics[width=3.5in, height = 4in]{Figures/poptopicsex-1}
\includegraphics[width=3.5in, height = 4in]{Figures/poptopicsex-2}
\includegraphics[width=3.5in, height = 4in]{Figures/poptopicsex-3}
\includegraphics[width=3.5in, height = 4in]{Figures/poptopicsex-4}
\caption{Population: Topic}
\label{fig:populationtopic}
\end{figure}


\newpage
\subsection*{Male vs Female}

<!--
### Occupation: Male vs Female
-->

```{r scatterplotmvfocc, fig.cap="Occupation: Male vs Female", include=FALSE}
g57 %>%
  pivot_wider(names_from = sex,
...
```{r lmmvfocc, fig.cap="Occupation: Male vs Female(linear model)", include=FALSE}
g57occmfnest <- g57 %>%
  pivot_wider(names_from = sex,
              values_from = count_occupation) %>%
...
```{r scatterplotmvfind, fig.cap="Industry: Male vs Female", include=FALSE}
g51 %>%
  pivot_wider(names_from = sex,
...
```{r lmmvfind, fig.cap="Industry: Male vs Female(linear model)", include=FALSE}
g51indmfnest <- g51 %>%
  pivot_wider(names_from = sex,
              values_from = count_industry) %>%
  select(industry, `F`, M ) %>%
...
<!--
### Field: Male vs Female
-->

```{r scatterplotmvffield, fig.cap="Field: Male vs Female", include=FALSE}
g47 %>%
  pivot_wider(names_from = sex,
              values_from = count_field) %>%
  ggplot(mapping = aes(x = M, y = `F`, colour = field)) +
  geom_point() +
  labs(title = "Population: Male vs Female", 
       x = "Male Population",
       y= "Female Population") +
...
```{r lmmvffield, fig.cap="Field: Male vs Female(linear model)", include=FALSE}
g47fieldmfnest <- g47 %>%
  pivot_wider(names_from = sex,
              values_from = count_field) %>%
  select(field, `F`, M ) %>%
  ...
  mutate(aug = map(model, broomstick::augment)) %>%
  unnest(aug)
mvf <- ggplot(g47fieldmfnest ,
       aes(x = M)) +
# index represent splitting value,
#geom_point(aes(y = `F`, colour = industry, aplha=0)) +
geom_line(aes(y = .fitted, colour = field)) +
    geom_abline(slope = 1) +
  theme(legend.position = "bottom")  +
  labs(title = "Population: Male vs Female", 
       x = "Male Population",
...
```{r scatterplotmvfedu, fig.cap="Education Level: Male vs Female", include=FALSE}
g46a %>%
  pivot_wider(names_from = sex,
              values_from = count_edu_lvl) %>%
  ggplot(mapping = aes(x = M, y = `F`, colour = educationlevel)) +
  geom_point() +
  labs(title = "Population: Male vs Female", 
       x = "Male Population",
       y= "Female Population") +
...
```{r lmmvfedu, fig.cap="Education Level: Male vs Female(linear model)", include=FALSE}
g46fieldmfnest <- g46a %>%
  pivot_wider(names_from = sex,
              values_from = count_edu_lvl) %>%
  select(educationlevel, `F`, M ) %>%
  ...
  mutate(aug = map(model, broomstick::augment)) %>%
  unnest(aug)
mvf <- ggplot(g46fieldmfnest ,
       aes(x = M)) +
# index represent splitting value,
#geom_point(aes(y = `F`, colour = industry, aplha=0)) +
geom_line(aes(y = .fitted, colour = educationlevel)) +
    geom_abline(slope = 1) +
  theme(legend.position = "bottom")  +
  labs(title = "Population: Male vs Female", 
       x = "Male Population",
...
A linear model to determine the male to female population ratio is fit for the male and female population with respect to occupations and industries. A line(slope=1, black line) decides the value of this ratio. The models above this line have a higher female population and the models below this line have a higher male population. These models are shown in Figure \ref{fig:lmmvfedu}, Figure \ref{fig:lmmvfocc}, Figure \ref{fig:lmmvffield} and Figure \ref{fig:lmmvfind}. 

<!--
Education level
-->

In Victoria, for any particular education level, more women have achieved it than men.
More women are educated(ratio F:M):
...
\begin{figure}[H]
\includegraphics[width=6in, height = 2in]{Figures/lmmvfedu-1}
\caption{Education Level: Male vs Female(linear model)}
\label{fig:lmmvfedu}
\end{figure}
...
\begin{figure}[H]
\includegraphics[width=6in, height = 2in]{Figures/lmmvfocc-1}
\caption{Occupation: Male vs Female(linear model)}
\label{fig:lmmvfocc}
\end{figure}
...
\begin{figure}[H]
\includegraphics[width=6in, height = 2in]{Figures/lmmvffield-1}
\caption{Field: Male vs Female(linear model)}
\label{fig:lmmvffield}
\end{figure}
...
Industry
-->
More women are employed in(ratio F:M):
...
\begin{figure}[H]
\includegraphics[width=6in, height = 2in]{Figures/lmmvfind-1}
\caption{Industry: Male vs Female(linear model)}
\label{fig:lmmvfind}
\end{figure}


\newpage
\section{Victoria Population: Age}
...
```{r networkeduage, fig.cap="Network: Education and Age", include=FALSE}
popeduage <- g46a %>%
...
```{r networkindage, fig.cap="Network: Industry and Age", include=FALSE}
popindage <- g51 %>%
...
### Population by Field
-->
```{r networkfieldage, fig.cap="Network: Field and Age", include=FALSE}
popfieldage <- g47 %>%
...
```{r networkoccage, fig.cap="Network: Occupation and Age", include=FALSE}
popoccage <- g57 %>%
...
networkoccage %>%
  ...
  geom_node_point(aes(size = 2, colour =category) )+
  geom_node_text(aes(label = name), repel = TRUE,  point.padding = unit(0.15, "lines")) +
  ggtitle("Occupation and Age") +
theme_void() + 
  theme(legend.position = "none")
```

A network plot is a representation of the relationship between vertices and the strength of this relationship is determined by the edge weight(width, opacity, length, etc). In order to understand the relation between the topics and the age, the Figure \ref{fig:networktopicage} is a network plot representing the relationships between the sub-divisions within each topic and the age of the people. The edge weight is determined by the population size that belongs to that connection.The network graphs are based on the population and distribution can be compared only within each age group since different age groups have different populations.
...
\begin{figure}[H]
\includegraphics[width=3.5in, height = 4in]{Figures/networkeduage-1}
\includegraphics[width=3.5in, height = 4in]{Figures/networkfieldage-1}
\includegraphics[width=3.5in, height = 4in]{Figures/networkindage-1}
\includegraphics[width=3.5in, height = 4in]{Figures/networkoccage-1}
\caption{Network: Topic and Age}
\label{fig:networktopicage}
\end{figure}
...
```{r popareaedu, fig.cap="Education Level: Region", include=FALSE}
popareaedu <- g46a %>%
...
```{r popareaind, fig.cap="Industry: Region", include=FALSE}
popindarea <- g51 %>%
...
```{r popareafield, fig.cap="Field: Region", include=FALSE}
popareafield <- g47 %>%
...
```{r popareaocc, fig.cap="Occupation: Region", include=FALSE}
popoccarea <- g57 %>%
...
The bar plots represent the SA4 regions and its working population with respect to their education levels, field of study, industry of employment and occupations. Each plot shows the region which had the highest population belonging to that subdivision(Figure \ref{fig:popareatopic}). 
...
\begin{figure}[H]
\includegraphics[width=3.5in, height = 4in]{Figures/popareaedu-1}
\includegraphics[width=3.5in, height = 4in]{Figures/popareaind-1}
\includegraphics[width=3.5in, height = 4in]{Figures/popareafield-1}
\includegraphics[width=3.5in, height = 4in]{Figures/popareaocc-1}
\caption{Sub-divisions and SA4 Regions}
\label{fig:popareatopic}
\end{figure}
...
```{r edmap, fig.cap="Spatial Education Level Distribution", include=FALSE}
...
popareaedumax %>% 
  ...
  ggtitle("Spatial Education Level Distribution") +
  theme_void() +
  scale_fill_brewer() +
  theme(legend.position = "bottom")
```
...
### Industry: Region
-->
```{r indmap, fig.cap="Spatial Industry Distribution", include=FALSE}
popindarea <- g51 %>%
  group_by(SA4_CODE_2016, industry) %>%
  summarise(count_indarea = sum(count_industry)) %>%
  ungroup()
...
popindareamax %>% 
  ...
  ggtitle("Spatial Industry Distribution") +
  theme_void() +
  scale_fill_brewer() +
  theme(legend.position = "bottom")
...
```{r fieldmap, fig.cap="Spatial Study Field Distribution", include=FALSE}
popareafield <- g47 %>%
  group_by(SA4_CODE_2016, field) %>%
  summarise(count_fieldarea = sum(count_field)) %>%
  ungroup()
...
popareafieldmax %>% 
  ...
  ggtitle("Spatial Study Field Distribution") +
  theme_void() +
  scale_fill_brewer() +
  theme(legend.position = "bottom")
...
```{r occmap, fig.cap="Spatial Occupation Distribution", include=FALSE}
...
popoccareamax %>% 
  ...
  ggtitle("Spatial Occupation Distribution") +
  theme_void() +
  scale_fill_brewer() +
  theme(legend.position = "bottom")
...
The maps \ref{fig:bestsubdivision} represent the SA4 regions and the distribution of population by their education levels, industries, field of study and occupations respectively.
...
\begin{figure}[H]
\includegraphics[width=3.5in, height = 4in]{Figures/edmap-1}
\includegraphics[width=3.5in, height = 4in]{Figures/indmap-1}
\includegraphics[width=3.5in, height = 4in]{Figures/fieldmap-1}
\includegraphics[width=3.5in, height = 4in]{Figures/occmap-1}
\caption{Maps: Best Sub-division for each Region}
\label{fig:bestsubdivision}
\end{figure}
...
```{r networkeduarea, include=FALSE}
popeduarea <- g46a %>%
 group_by(SA4_CODE_2016, afq_level) %>%
  summarise(count_eduarea = sum(count_edu_lvl)) %>%
  ungroup()
...
networkeduarea %>%
  ...
  geom_node_point(aes(size = 2, colour =category) )+
  geom_node_text(aes(label = name), repel = TRUE,  point.padding = unit(0.15, "lines")) +
  ggtitle("Education Level and Region") +
  theme_void() +
  theme(legend.position = "none")
```
...
```{r networkindarea, include=FALSE}
popindarea <- g51 %>%
  group_by(SA4_CODE_2016, industry) %>%
  summarise(count_indarea = sum(count_industry)) %>%
  ungroup()
...
```{r networkfieldarea, include=FALSE}
popfieldarea <- g47 %>%
 group_by(SA4_CODE_2016, field) %>%
  summarise(count_fieldarea = sum(count_field)) %>%
  ungroup()
...
```{r networkoccarea, include=FALSE}
popareaocc <- g57 %>%
  group_by(SA4_CODE_2016, occupation) %>%
  summarise(count_occarea = sum(count_occupation)) %>%
  ungroup()
...
networkoccarea %>%
  ...
  geom_node_point(aes(size = 2, colour =category) )+
  geom_node_text(aes(label = name), repel = TRUE,  point.padding = unit(0.15, "lines")) +
  ggtitle("Occupation and Region") +
  theme_void() +
  theme(legend.position = "none")
```

The network graphs \ref{fig:areatopic} represent the relationship between the SA4 regions and the population with respect to the education levels, Industry, Field of Study and Occupations. For analysis, we grouped the SA4 region codes with education levels, industry, occupation and fields of study, then created a data frame for nodes and joined both data frames to create the network graphs.


From the network graphs it can be observed that
 - Region 206 has most population for all sectors throughout the graphs.
 - Highest number of people have studied Management and Commerce.
 - For Industries, Health care and Professional and scientific services accounted for highest population.
 - Highest number of employees were employed as Professionals and Managers.
 
\begin{figure}[H]
\includegraphics[width=3.5in, height = 4in]{Figures/networkeduarea-1}
\includegraphics[width=3.5in, height = 4in]{Figures/networkindarea-1}
\includegraphics[width=3.5in, height = 4in]{Figures/networkfieldarea-1}
\includegraphics[width=3.5in, height = 4in]{Figures/networkoccarea-1}
\caption{Network: SA4 Regions and Topic}
\label{fig:areatopic}
\end{figure}

\newpage
\section{Working Hours and Age in Industries and Occupation}
...
```{r hrplots, fig.show='hold', out.width="50%", include=FALSE}
p1 <- g52 %>%  
  ggplot(g52, 
...
- It can be observed from Figure \ref{fig:wrkhrcmpind} that overall females worked more than men. However, as the number of work-hours increased men have worked more than women.

\begin{figure}[H]
\includegraphics[width=3.5in, height = 2in]{Figures/hrplots-1}
\includegraphics[width=3.5in, height = 2in]{Figures/hrplots-2}
\caption{Working hour comparison: Industry}
\label{fig:wrkhrcmpind}
\end{figure}
...
- It can be observed from Figure \@ref(fig:indhrs) that industries like health care, education and training, construction and Professional and technical services have more working population as the working hours increased. Mining, electricity, gas, water showed low working population irrespective of work hours.
- From Figure \@ref(fig:indages) it is observed that people in the age group [15,24] work more in accomodation and food services and retail trade. A probable reason is part-time job for the students. The hours worked between 1 to 15 hours also has a higher population in these industries.

```{r indhrs, fig.cap="Population: Industries and hours"}
...
```{r indages, fig.cap="Population: Industries and Age Groups"}
...
```{r hrsplots, fig.show='hold', out.width="50%", fig.cap="", include=FALSE}
p3 <- g58 %>% 
  ggplot(g58, 
...
It can be observed from Figure \ref{fig:wrkhrcmpocc} that overall females worked more than men in all occupations. Although, for maximum hours worked, as number of working-hours increased, the number of men and women remained the same.

\begin{figure}[H]
\includegraphics[width=3.5in, height = 3in]{Figures/hrsplots-1}
\includegraphics[width=3.5in, height = 3in]{Figures/hrsplots-2}
\caption{Working hour comparison: Occupation}
\label{fig:wrkhrcmpocc}
\end{figure}
...
- General @ABS 
- Geopacks @absgeopack 
- Datapacks @absdatapack 
- Australian Statistical Geography Standard (ASGS) @absasgs 
- Data Cleaning Reference @datacleaning 
