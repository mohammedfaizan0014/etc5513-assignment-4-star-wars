...
```{r bestedu, fig.cap="Education Level: Region"}
popareaedu <- g46a %>%
...
```{r popareaind, fig.cap="Industry: Region"}
popindarea <- g51 %>%
...
```{r bestfield, fig.cap="Field: Region"}
popareafield <- g47 %>%
...
```{r popareaocc, fig.cap="Occupation: Region"}
popoccarea <- g57 %>%
...
```{r hrplots, fig.show='hold', out.width="50%"}
p1 <- g52 %>%  
  ggplot(g52, 
...
```{r hrsplots, fig.show='hold', out.width="50%"}
p3 <- g58 %>% 
  ggplot(g58, 
